-- REAL FIX: Remove date_range variables that conflict with RETURNS TABLE columns
-- The RETURNS TABLE creates implicit variables that conflict with table column references
...
BEGIN
  ...
  -- Calculate range inline to avoid variable name conflicts
  RETURN QUERY
  SELECT
    cs.id,
    cs.session_type,
  ...
  WHERE cs.session_date >= base_date
    AND cs.session_date < base_date + (weeks_ahead * 7)
...
SELECT 'Function REALLY fixed this time!' as status;
