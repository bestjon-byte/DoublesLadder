-- FINAL FIX: Remove column aliases that match RETURNS TABLE column names
-- PostgreSQL maps by position, not by name, so aliases are unnecessary and cause ambiguity
...
BEGIN
  ...
  -- NO ALIASES - PostgreSQL maps columns by position to the RETURNS TABLE
  RETURN QUERY
  SELECT
    cs.id,
    cs.session_type,
...
SELECT 'Function fixed - aliases removed!' as status;
