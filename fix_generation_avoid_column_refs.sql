-- ULTIMATE FIX: Avoid ALL references to problematic column names in WHERE
-- Store everything in v_ variables and use ONLY those in comparisons
...
BEGIN
  ...
  -- Return results by looping through and returning each row
  -- This completely avoids column name references in WHERE clauses
  FOR v_row IN
    SELECT
      cs.id,
      cs.session_type,
  ...
  LOOP
    -- Filter in PL/pgSQL using v_ variables instead of SQL WHERE
    IF v_row.session_date >= v_base_date AND v_row.session_date < v_end_date THEN
      session_id := v_row.id;
      session_type := v_row.session_type;
      session_date := v_row.session_date;
      session_time := v_row.session_time;
      RETURN NEXT;
    END IF;
  END LOOP;
...
SELECT 'Function fixed - filtering in PL/pgSQL loop!' as status;
