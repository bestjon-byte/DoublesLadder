...
export const useApp = (userId, selectedSeasonId) => {
  ...
  const deleteSeason = useCallback(async (seasonId, seasonName) => {
    try {
      // Deleting season with ELO restoration
      ...
      // Found season players to process for ELO restoration
      ...
      if (seasonPlayers && seasonPlayers.length > 0) {
        // Starting ELO restoration process
        ...
        for (const seasonPlayer of seasonPlayers) {
          ...
          if (restoreError) {
            ...
            // Player ELO restored in all active seasons
          } else {
            // No previous ELO history found to restore
          }
        }
      }
      ...
      // Deleting trophy cabinet entries
      ...
      // Successfully deleted season with ELO restoration
