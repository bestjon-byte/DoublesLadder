...
import { Users, Calendar, Trophy, Settings, User, Award, GraduationCap, PoundSterling } from 'lucide-react';
...
const Navigation = ({ activeTab, setActiveTab, currentUser, ladderPlayers, selectedSeason }) => {
  ...
  // Coaches get Sessions and Payments tabs
  if (isCoach) {
    const coachTabs = [
      { id: 'coaching', icon: GraduationCap, label: 'Sessions' },
      { id: 'coach-payments', icon: PoundSterling, label: 'Payments' },
    ];

    return (
      <>
        {/* Desktop Navigation for coach */}
        <nav className="hidden sm:block bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex">
              {coachTabs.map(({ id, icon: Icon, label }) => (
                <button
                  key={id}
                  onClick={() => {
                    haptics.tap();
          ...
          <div className="flex">
            {coachTabs.map(({ id, icon: Icon, label }) => (
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-2 min-h-[64px] transition-colors relative ${
                  activeTab === id
